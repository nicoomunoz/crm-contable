...
export default async function TramitesPage() {
  try {
    ...
    // Consulta 1: Trámites
    const { data: rawTramites, error: errorTramites } = await supabase
      .from('tramites')
      .select('*')
      .order('created_at', { ascending: false })
    
    // Consulta 2: Clientes
    const { data: rawClientes, error: errorClientes } = await supabase
      .from('clientes')
      .select('id, razon_social')

    // ✅ VERIFICAR ERRORES EXPLÍCITAMENTE
    if (errorTramites) throw new Error(`Error tramites: ${errorTramites.message}`)
    if (errorClientes) throw new Error(`Error clientes: ${errorClientes.message}`)
    ...
    return (
      <div className="space-y-6 pb-20">
        ...
        <div className="grid gap-4">
          {tramites.length === 0 ? (
            <p className="text-slate-400 text-center py-8">Sin trámites registrados</p>
          ) : (
            tramites.map((t: any) => {
    ...
    console.error('Error en TramitesPage:', error)
    return (
      <div className="p-6 bg-red-50 rounded-2xl border border-red-200">
        <h2 className="text-red-600 font-black">Error cargando trámites</h2>
        <p className="text-red-500 text-sm mt-2">{error instanceof Error ? error.message : 'Error desconocido'}</p>
      </div>
    )
  }
}
